...
//	ArticleApiController에서 사용하던 ArticleRepository는 service 계층으로 넘기고 ArticleRepository는
//	service 객체를 만들어 사용한다.
...
//	form에서 데이터를 받아올 때는 커맨드 객체로 받으면 되지만 REST Api에서 JSON으로 던지는 데이터를 받을 때는
//	body 부분에 담겨서 넘어오는 데이터를 받아야 하므로 커맨드 객체 앞에 @RequestBody 어노테이션을 붙여서 받는다.
//	상태 코드를 저장해서 리턴하려면 ResponseEntity 객체를 사용한다.
	public ResponseEntity<Article> create(@RequestBody ArticleForm dto) {
		log.info("ArticleApiController의 create() 메소드 실행");
		// log.info("dto: " + dto); // dto: ArticleForm(id=null, title=금요일, content=금요일 좋아)
...
//	글 수정
	...
	public ResponseEntity<Article> update(@PathVariable Long id, @RequestBody ArticleForm dto) {
		...
		// 글 수정 후 정상 응답을 보낸다.
		// @RequestBody를 통해 ArticleForm으로 넘어오는 수정할 데이터의 일부가 넘어오지 않으면 넘어오지 않은 데이터는
		// null로 수정되기 때문에 이를 방지하기 위해 조회된 수정할 entity를 RequestBody를 통해 넘어온 데이터가
		// 있는 경우만 수정한다.
		...
		*/
		...
		Article updated = articleService.update(id, dto);
		return updated != null ?
			ResponseEntity.status(HttpStatus.OK).body(updated) : // HttpStatus.OK => 200 성공 요청이 나온다.
			ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
	}
